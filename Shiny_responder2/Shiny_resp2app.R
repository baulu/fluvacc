...
influenza_comb <- influenza_folds_fullid %>% 
  left_join(influenza_folds2) %>% 
  select(Pat_ID, strain, assay, fold_change, baseline_titer, beyer_sum_rank, classification_d20 = beyer_sum_d20, classification_d30 = beyer_sum_d30) %>% 
  print()
...
server <- function(input, output, session) {
  ...
  output$classification <- renderUI({
    ...
    selectInput("classification", "Select Classification Category:",
                choices = unique(influenza_comb[[classification_column]]),
                selected = unique(influenza_comb[[classification_column]])[1])
  })
  
  # Get the list of unique patient IDs from the data, filtered by the selected classification category
  filtered_patients <- reactive({
    classification_column <- input$classification_type  # Select column based on input
    patient_ids <- influenza_comb %>%
